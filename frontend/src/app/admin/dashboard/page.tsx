// ===== JanSathi AI — Admin Dashboard (CommPulse) =====
// Analytics dashboard — ADMIN ONLY. Protected by:
//   1. Next.js Edge Middleware (server-side session + role check)
//   2. AuthGuard component (client-side UX guard)
// Voice Power Orb activates when voice mode is detected.
...
// Force dynamic rendering — admin route must never be statically prerendered
export const dynamic = "force-dynamic";

// ── Dynamic 3D import (no SSR — prevents hydration mismatch) ────
const VoicePowerOrb = dynamicImport(
    () => import("@/components/3d/VoicePowerOrb"),
...
> = {
    ...
    jankrishi: { name: "JanKrishi", nameHi: "जनकृषि", color: "#10B981", icon: "🌾" },
    janvyapar: { name: "JanVyapar", nameHi: "जनव्यापार", color: "#F59E0B", icon: "💼" },
    jankaushal: { name: "JanKaushal", nameHi: "जनकौशल", color: "#EF4444", icon: "🛠️" },
};
            ...
            >
                ...
                >
                    <div className="flex items-center gap-3">
                        <Link href="/chat" className="p-1" title="Back to Chat">
                        ...
                        <div>
                            ...
                            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                                सामुदायिक प्रभाव विश्लेषण · Admin Only
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        {/* Voice Orb Toggle — demo control for admin */}
                        <button
                            onClick={() => setVoiceActive((v) => !v)}
                            className="p-2 rounded-lg flex items-center gap-1.5 text-xs font-medium"
                            title="Toggle Voice Power Orb"
                            style={{
                                background: voiceActive ? "#2563EB20" : "var(--bg-surface)",
                                border: voiceActive
                                    ? "1px solid #2563EB50"
                                    : "1px solid var(--border-primary)",
                                color: voiceActive ? "#3B82F6" : "var(--text-secondary)",
                            }}
                        >
                            <Mic size={14} />
                            <span>Voice Orb</span>
                ...
                {/* ── Voice Power Orb (behind all content) ──────────── */}
                <div
                    style={{
                        position: "fixed",
                        inset: 0,
                ...
                >
                    <VoicePowerOrb isActive={voiceActive} />
                </div>

                {/* ── Dashboard Content (above orb) ─────────────────── */}
                    ...
                    {loading && !data ? (
                        ...
                        <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto">
                            ...
                            <p
                                ...
                                style={{ color: "var(--text-muted)" }}
                            >
                                CommPulse — सामुदायिक प्रभाव मापक · Admin Console
                            </p>
                        </div>
                    ) : (
                        <ErrorFallback
